...
describe('useStakesMachine', () => {
  it('back adds to support pool', async () => {
    ...
    await act(async () => { await result.current.back('newbacker', 'New Backer', 10) })
    await waitFor(() => expect(result.current.pools.support).toBe(before + 10))
  })
  it('fade adds to doubt pool', async () => {
    const { result } = renderHook(() => useStakesMachine())
    const before = result.current.pools.doubt
    await act(async () => { await result.current.fade('skeptic', 'Skeptic', 8) })
    await waitFor(() => expect(result.current.pools.doubt).toBe(before + 8))
  })
